...
CREATE TABLE `d_client_supplier` (
  ...
  `status` char(1) NOT NULL DEFAULT '0' COMMENT '状态0存在1不存在',
  `client_us` char(1) NOT NULL DEFAULT '2' COMMENT '0供应商1客户2都有',
  KEY `belong` (`belong`),
  KEY `id` (`id`),
...
CREATE TABLE `d_grant` (
  ...
  CONSTRAINT `fk_d_grant_d_staff_login_1` FOREIGN KEY (`id`) REFERENCES `d_staff_login` (`jobnumber`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE TABLE `d_organization` (
  ...
  `belong` varchar(30) NOT NULL DEFAULT '0000' COMMENT '上级',
  `number` char(7) DEFAULT NULL COMMENT '人数',
  `who` char(1) NOT NULL COMMENT '0员工结构1产品结构2客户结构3供应商结构',
  `status` char(1) NOT NULL COMMENT '状态0存在1删除',
  PRIMARY KEY (`id`),
  UNIQUE KEY `one` (`name`),
  KEY `belong` (`belong`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
insert  into `d_organization`(`id`,`name`,`belong`,`number`,`who`,`status`) values ('C-111','公司','1','1','0','');
...
CREATE TABLE `d_purchase` (
  ...
  `newtime` datetime NOT NULL ON UPDATE CURRENT_TIMESTAMP COMMENT '制单时间',
  `user_id` varchar(30) NOT NULL COMMENT '制单人',
  `money` double NOT NULL COMMENT '总价',
  `get_time` date NOT NULL COMMENT '交货时间',
  `delivery_type` char(1) NOT NULL COMMENT '交货方式：0现取1物流',
  `payment` char(1) NOT NULL COMMENT '支付方式0现付',
  `in_address` varchar(60) NOT NULL COMMENT '收获地址',
  ...
  CONSTRAINT `fk_table_2_d_staff_login_1` FOREIGN KEY (`user_id`) REFERENCES `d_staff_login` (`jobnumber`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE TABLE `d_role` (
  ...
  `m_id` varchar(30) NOT NULL COMMENT '模块id',
...
CREATE TABLE `d_staff_clock` (
  `staff_id` char(10) NOT NULL COMMENT '员工工号',
  `to_work` date NOT NULL COMMENT '上班时间',
  `off_work` date NOT NULL COMMENT '下班时间',
  `to_work_info` char(1) NOT NULL DEFAULT '0' COMMENT '0正常1迟到2矿工3请假',
  `off_work_info` char(1) NOT NULL DEFAULT '0' COMMENT '0正常1早退3请假',
...
CREATE TABLE `d_staff_information` (
  ...
  `location` varchar(100) NOT NULL COMMENT '家庭住址',
...
insert  into `d_staff_information`(`name`,`age`,`telephone`,`identity_card`,`mail`,`location`,`h_name`,`h_telephone`,`id`) values ('admin','1','18038848740','111111111111111','1440705477@qq.com','jj','1','1',1);
...
CREATE TABLE `d_staff_login` (
  ...
  `jobnumber` char(10) NOT NULL COMMENT '工号（系统生成）',
  `in_time` date DEFAULT NULL COMMENT '入职时间',
  `out_time` date DEFAULT NULL COMMENT '离职时间',
  `belong` varchar(30) NOT NULL COMMENT '上级部门',
  PRIMARY KEY (`id`),
...
CREATE TABLE `d_warehouse` (
  ...
  `name` varchar(30) NOT NULL COMMENT '仓库名',
  `location` varchar(40) NOT NULL COMMENT '位置',
  `status` char(1) NOT NULL DEFAULT '0' COMMENT '0存在1消失',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
